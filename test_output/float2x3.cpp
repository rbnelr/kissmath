...
namespace kissmath {
	...
	float const& float2x3::get (int r, int c) const {
		return arr[c][r];
	}
	
	// get matrix column
	float2 const& float2x3::get_column (int indx) const {
		return arr[indx];
	}
	
	// get matrix row
	float3 float2x3::get_row (int indx) const {
	...
	float2x3::float2x3 (float c00, float c01, float c02,
	...
	float2x3 float2x3::rows (float3 row0, float3 row1) {
		return float2x3(row0[0], row0[1], row0[2],
						row1[0], row1[1], row1[2]);
	}
	
	// supply all cells in row major order
	float2x3 float2x3::rows (float c00, float c01, float c02,
							 float c10, float c11, float c12) {
	...
	float2x3 float2x3::columns (float2 col0, float2 col1, float2 col2) {
		return float2x3(col0[0], col1[0], col2[0],
						col0[1], col1[1], col2[1]);
	}
	
	// supply all cells in column major order
	float2x3 float2x3::columns (float c00, float c10,
								float c01, float c11,
								float c02, float c12) {
		return float2x3(c00, c01, c02,
						c10, c11, c12);
	...
	float2x3::operator float2x2 () const {
		return float2x2(arr[0][0], arr[1][0],
						arr[0][1], arr[1][1]);
	}
	
	// extend/truncate matrix of other size
	float2x3::operator float3x3 () const {
	...
	// extend/truncate matrix of other size
	float2x3::operator float4x4 () const {
	...
	// extend/truncate matrix of other size
	float2x3::operator float3x4 () const {
	...
	// typecast
	float2x3::operator double2x3 () const {
		return double2x3((double)arr[0][0], (double)arr[0][1], (double)arr[0][2],
						 (double)arr[1][0], (double)arr[1][1], (double)arr[1][2]);
	}
	...
	float2x3& float2x3::operator+= (float r) {
		*this = *this + r;
		return *this;
	}
	...
	float2x3& float2x3::operator*= (float2x3 const& r) {
		*this = *this * r;
		return *this;
	}
	...
	float2 operator* (float2x3 const& l, float3 r) {
		return float2(l.arr[0].x * r.x + l.arr[1].x * r.y + l.arr[2].x * r.z,
					  l.arr[0].y * r.x + l.arr[1].y * r.y + l.arr[2].y * r.z);
	}
	
	// vector-matrix multiply
	float3 operator* (float2 l, float2x3 const& r) {
		return float3(l.x * r.arr[0].x + l.y * r.arr[0].y,
