...
namespace kissmath {
	...
	uint16& uint16v2::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	uint16 const& uint16v2::operator[] (int i) const {
		return arr[i];
	}
	...
	uint16v2::uint16v2 (uint16 all): x{all}, y{all} {
		
	}
	
	// supply all components
	uint16v2::uint16v2 (uint16 x, uint16 y): x{x}, y{y} {
		
	}
	
	// truncate vector
	uint16v2::uint16v2 (uint16v3 v): x{v.x}, y{v.y} {
		
	}
	
	// truncate vector
	uint16v2::uint16v2 (uint16v4 v): x{v.x}, y{v.y} {
	...
	// type cast operator
	uint16v2::operator bool2 () const {
		return bool2((bool)x, (bool)y);
	}
	
	// type cast operator
	uint16v2::operator float2 () const {
		return float2((float)x, (float)y);
	}
	
	// type cast operator
	uint16v2::operator double2 () const {
		return double2((double)x, (double)y);
	}
	
	// type cast operator
	uint16v2::operator int8v2 () const {
		return int8v2((int8)x, (int8)y);
	}
	
	// type cast operator
	uint16v2::operator int16v2 () const {
		return int16v2((int16)x, (int16)y);
	}
	
	// type cast operator
	uint16v2::operator int2 () const {
		return int2((int)x, (int)y);
	}
	
	// type cast operator
	uint16v2::operator int64v2 () const {
		return int64v2((int64)x, (int64)y);
	}
	
	// type cast operator
	uint16v2::operator uint8v2 () const {
		return uint8v2((uint8)x, (uint8)y);
	}
	
	// type cast operator
	uint16v2::operator uint2 () const {
		return uint2((uint)x, (uint)y);
	}
	
	// type cast operator
	uint16v2::operator uint64v2 () const {
		return uint64v2((uint64)x, (uint64)y);
	}
	
	
	// componentwise arithmetic operator
	uint16v2 uint16v2::operator+= (uint16v2 r) {
		x += r.x;
		y += r.y;
		return *this;
	}
	...
	uint16v2 operator+ (uint16v2 v) {
		return uint16v2(+v.x, +v.y);
	}
	
	uint16v2 operator+ (uint16v2 l, uint16v2 r) {
		return uint16v2(l.x + r.x, l.y + r.y);
	}
	...
	uint16v2 select (bool2 c, uint16v2 l, uint16v2 r) {
		return uint16v2(c.x ? l.x : r.x, c.y ? l.y : r.y);
	}
	...
	uint16v2 min (uint16v2 l, uint16v2 r) {
		return uint16v2(min(l.x,r.x), min(l.y,r.y));
	}
	
	// componentwise maximum
	uint16v2 max (uint16v2 l, uint16v2 r) {
		return uint16v2(max(l.x,r.x), max(l.y,r.y));
	}
	...
	uint16 min_component (uint16v2 v, int* min_index) {
		int index = 0;
		uint16 min_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] <= min_val) {
	...
	uint16 max_component (uint16v2 v, int* max_index) {
		int index = 0;
		uint16 max_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] >= max_val) {
	...
	uint16v2 wrap (uint16v2 v, uint16v2 range) {
		return uint16v2(wrap(v.x,range.x), wrap(v.y,range.y));
	}
	
	// componentwise wrap
	uint16v2 wrap (uint16v2 v, uint16v2 a, uint16v2 b) {
		return uint16v2(wrap(v.x,a.x,b.x), wrap(v.y,a.y,b.y));
	}
