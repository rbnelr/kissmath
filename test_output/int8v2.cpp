// file was generated by kissmath.py at <TODO: add github link>
#include "int8v2.hpp"

#include "uint8v2.hpp"
#include "double2.hpp"
#include "float2.hpp"
#include "int8v4.hpp"
#include "int2.hpp"
#include "int16v2.hpp"
#include "int64v2.hpp"
#include "uint16v2.hpp"
#include "bool2.hpp"
#include "uint2.hpp"
#include "uint64v2.hpp"
#include "int8v3.hpp"

namespace kissmath {
	//// forward declarations
	// typedef these because the _t suffix is kinda unwieldy when using these types often
	
	typedef uint8_t uint8;
	typedef int8_t int8;
	typedef int16_t int16;
	typedef int64_t int64;
	typedef uint16_t uint16;
	typedef unsigned int uint;
	typedef uint64_t uint64;
	typedef int8_t int8;
	
	// Component indexing operator
	int8& int8v2::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	int8 const& int8v2::operator[] (int i) const {
		return arr[i];
	}
	
	
	// uninitialized constructor
	int8v2::int8v2 () {
		
	}
	
	// sets all components to one value
	// implicit constructor -> float3(x,y,z) * 5 will be turned into float3(x,y,z) * float3(5) by to compiler to be able to execute operator*(float3, float3), which is desirable
	// and short initialization like float3 a = 0; works
	int8v2::int8v2 (int8 all): x{all}, y{all} {
		
	}
	
	// supply all components
	int8v2::int8v2 (int8 x, int8 y): x{x}, y{y} {
		
	}
	
	// truncate vector
	int8v2::int8v2 (int8v3 v): x{v.x}, y{v.y} {
		
	}
	
	// truncate vector
	int8v2::int8v2 (int8v4 v): x{v.x}, y{v.y} {
		
	}
	
	//// Truncating cast operators
	
	
	//// Type cast operators
	
	
	// type cast operator
	int8v2::operator bool2 () const {
		return bool2((bool)x, (bool)y);
	}
	
	// type cast operator
	int8v2::operator float2 () const {
		return float2((float)x, (float)y);
	}
	
	// type cast operator
	int8v2::operator double2 () const {
		return double2((double)x, (double)y);
	}
	
	// type cast operator
	int8v2::operator int16v2 () const {
		return int16v2((int16)x, (int16)y);
	}
	
	// type cast operator
	int8v2::operator int2 () const {
		return int2((int)x, (int)y);
	}
	
	// type cast operator
	int8v2::operator int64v2 () const {
		return int64v2((int64)x, (int64)y);
	}
	
	// type cast operator
	int8v2::operator uint8v2 () const {
		return uint8v2((uint8)x, (uint8)y);
	}
	
	// type cast operator
	int8v2::operator uint16v2 () const {
		return uint16v2((uint16)x, (uint16)y);
	}
	
	// type cast operator
	int8v2::operator uint2 () const {
		return uint2((uint)x, (uint)y);
	}
	
	// type cast operator
	int8v2::operator uint64v2 () const {
		return uint64v2((uint64)x, (uint64)y);
	}
	
	
	// componentwise arithmetic operator
	int8v2 int8v2::operator+= (int8v2 r) {
		x += r.x;
		y += r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	int8v2 int8v2::operator-= (int8v2 r) {
		x -= r.x;
		y -= r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	int8v2 int8v2::operator*= (int8v2 r) {
		x *= r.x;
		y *= r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	int8v2 int8v2::operator/= (int8v2 r) {
		x /= r.x;
		y /= r.y;
		return *this;
	}
	
	//// arthmethic ops
	
	
	int8v2 operator+ (int8v2 v) {
		return int8v2(+v.x, +v.y);
	}
	
	int8v2 operator- (int8v2 v) {
		return int8v2(-v.x, -v.y);
	}
	
	int8v2 operator+ (int8v2 l, int8v2 r) {
		return int8v2(l.x + r.x, l.y + r.y);
	}
	
	int8v2 operator- (int8v2 l, int8v2 r) {
		return int8v2(l.x - r.x, l.y - r.y);
	}
	
	int8v2 operator* (int8v2 l, int8v2 r) {
		return int8v2(l.x * r.x, l.y * r.y);
	}
	
	int8v2 operator/ (int8v2 l, int8v2 r) {
		return int8v2(l.x / r.x, l.y / r.y);
	}
	
	//// bitwise ops
	
	
	int8v2 operator~ (int8v2 v) {
		return int8v2(~v.x, ~v.y);
	}
	
	int8v2 operator& (int8v2 l, int8v2 r) {
		return int8v2(l.x & r.x, l.y & r.y);
	}
	
	int8v2 operator| (int8v2 l, int8v2 r) {
		return int8v2(l.x | r.x, l.y | r.y);
	}
	
	int8v2 operator^ (int8v2 l, int8v2 r) {
		return int8v2(l.x ^ r.x, l.y ^ r.y);
	}
	
	//// comparison ops
	
	
	// componentwise comparison returns a bool vector
	bool2 operator< (int8v2 l, int8v2 r) {
		return bool2(l.x < r.x, l.y < r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator<= (int8v2 l, int8v2 r) {
		return bool2(l.x <= r.x, l.y <= r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator> (int8v2 l, int8v2 r) {
		return bool2(l.x > r.x, l.y > r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator>= (int8v2 l, int8v2 r) {
		return bool2(l.x >= r.x, l.y >= r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator== (int8v2 l, int8v2 r) {
		return bool2(l.x == r.x, l.y == r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator!= (int8v2 l, int8v2 r) {
		return bool2(l.x != r.x, l.y != r.y);
	}
	
	// vectors are equal, equivalent to all(l == r)
	bool equal (int8v2 l, int8v2 r) {
		return all(l == r);
	}
	
	// componentwise ternary (c ? l : r)
	int8v2 select (bool2 c, int8v2 l, int8v2 r) {
		return int8v2(c.x ? l.x : r.x, c.y ? l.y : r.y);
	}
	
	//// misc ops
	
	// componentwise absolute
	int8v2 abs (int8v2 v) {
		return int8v2(abs(v.x), abs(v.y));
	}
	
	// componentwise minimum
	int8v2 min (int8v2 l, int8v2 r) {
		return int8v2(min(l.x,r.x), min(l.y,r.y));
	}
	
	// componentwise maximum
	int8v2 max (int8v2 l, int8v2 r) {
		return int8v2(max(l.x,r.x), max(l.y,r.y));
	}
	
	// componentwise clamp into range [a,b]
	int8v2 clamp (int8v2 x, int8v2 a, int8v2 b) {
		return min(max(x,a), b);
	}
	
	// componentwise clamp into range [0,1] also known as saturate in hlsl
	int8v2 clamp (int8v2 x) {
		return min(max(x, int8(0)), int8(1));
	}
	
	// get minimum component of vector, optionally get component index via min_index
	int8 min_component (int8v2 v, int* min_index) {
		int index = 0;
		int8 min_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] <= min_val) {
				index = i;
				min_val = v.arr[i];
			}
		}
		if (min_index) *min_index = index;
		return min_val;
	}
	
	// get maximum component of vector, optionally get component index via max_index
	int8 max_component (int8v2 v, int* max_index) {
		int index = 0;
		int8 max_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] >= max_val) {
				index = i;
				max_val = v.arr[i];
			}
		}
		if (max_index) *max_index = index;
		return max_val;
	}
	
	
	// componentwise wrap
	int8v2 wrap (int8v2 v, int8v2 range) {
		return int8v2(wrap(v.x,range.x), wrap(v.y,range.y));
	}
	
	// componentwise wrap
	int8v2 wrap (int8v2 v, int8v2 a, int8v2 b) {
		return int8v2(wrap(v.x,a.x,b.x), wrap(v.y,a.y,b.y));
	}
	
	
	//// Vector math
	
	
	// magnitude of vector
	float length (int8v2 v) {
		return sqrt((float)(v.x * v.x + v.y * v.y));
	}
	
	// squared magnitude of vector, cheaper than length() because it avoids the sqrt(), some algorithms only need the squared magnitude
	int8 length_sqr (int8v2 v) {
		return v.x * v.x + v.y * v.y;
	}
	
	// distance between points, equivalent to length(a - b)
	float distance (int8v2 a, int8v2 b) {
		return length(a - b);
	}
	
	// normalize vector so that it has length() = 1, undefined for zero vector
	float2 normalize (int8v2 v) {
		return float2(v) / length(v);
	}
	
	// normalize vector so that it has length() = 1, returns zero vector if vector was zero vector
	float2 normalizesafe (int8v2 v) {
		float len = length(v);
		if (len == float(0)) {
			return float(0);
		}
		return float2(v) / float2(len);
	}
	
	// dot product
	int8 dot (int8v2 l, int8v2 r) {
		return l.x * r.x + l.y * r.y;
	}
	
	// 2d cross product hack for convenient 2d stuff
	// same as cross({T.name[:-2]}3(l, 0), {T.name[:-2]}3(r, 0)).z,
	// ie. the cross product of the 2d vectors on the z=0 plane in 3d space and then return the z coord of that (signed mag of cross product)
	int8 cross (int8v2 l, int8v2 r) {
		return l.x * r.y - l.y * r.x;
	}
	
	// rotate 2d vector counterclockwise 90 deg, ie. int8v2(-y, x) which is fast
	int8v2 rotate90 (int8v2 v) {
		return int8v2(-v.y, v.x);
	}
}

