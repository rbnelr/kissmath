// file was generated by kissmath.py at <TODO: add github link>
#include "double2.hpp"

#include "float2.hpp"
#include "double3.hpp"
#include "uint16v2.hpp"
#include "uint64v2.hpp"
#include "int2.hpp"
#include "bool2.hpp"
#include "int8v2.hpp"
#include "int16v2.hpp"
#include "uint2.hpp"
#include "uint8v2.hpp"
#include "double4.hpp"
#include "int64v2.hpp"

namespace kissmath {
	//// forward declarations
	// typedef these because the _t suffix is kinda unwieldy when using these types often
	
	typedef uint16_t uint16;
	typedef uint64_t uint64;
	typedef int8_t int8;
	typedef int16_t int16;
	typedef unsigned int uint;
	typedef uint8_t uint8;
	typedef int64_t int64;
	
	// Component indexing operator
	double& double2::operator[] (int i) {
		return arr[i];
	}
	
	// Component indexing operator
	double const& double2::operator[] (int i) const {
		return arr[i];
	}
	
	
	// uninitialized constructor
	double2::double2 () {
		
	}
	
	// sets all components to one value
	// implicit constructor -> float3(x,y,z) * 5 will be turned into float3(x,y,z) * float3(5) by to compiler to be able to execute operator*(float3, float3), which is desirable
	// and short initialization like float3 a = 0; works
	double2::double2 (double all): x{all}, y{all} {
		
	}
	
	// supply all components
	double2::double2 (double x, double y): x{x}, y{y} {
		
	}
	
	// truncate vector
	double2::double2 (double3 v): x{v.x}, y{v.y} {
		
	}
	
	// truncate vector
	double2::double2 (double4 v): x{v.x}, y{v.y} {
		
	}
	
	//// Truncating cast operators
	
	
	//// Type cast operators
	
	
	// type cast operator
	double2::operator bool2 () const {
		return bool2((bool)x, (bool)y);
	}
	
	// type cast operator
	double2::operator float2 () const {
		return float2((float)x, (float)y);
	}
	
	// type cast operator
	double2::operator int8v2 () const {
		return int8v2((int8)x, (int8)y);
	}
	
	// type cast operator
	double2::operator int16v2 () const {
		return int16v2((int16)x, (int16)y);
	}
	
	// type cast operator
	double2::operator int2 () const {
		return int2((int)x, (int)y);
	}
	
	// type cast operator
	double2::operator int64v2 () const {
		return int64v2((int64)x, (int64)y);
	}
	
	// type cast operator
	double2::operator uint8v2 () const {
		return uint8v2((uint8)x, (uint8)y);
	}
	
	// type cast operator
	double2::operator uint16v2 () const {
		return uint16v2((uint16)x, (uint16)y);
	}
	
	// type cast operator
	double2::operator uint2 () const {
		return uint2((uint)x, (uint)y);
	}
	
	// type cast operator
	double2::operator uint64v2 () const {
		return uint64v2((uint64)x, (uint64)y);
	}
	
	
	// componentwise arithmetic operator
	double2 double2::operator+= (double2 r) {
		x += r.x;
		y += r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	double2 double2::operator-= (double2 r) {
		x -= r.x;
		y -= r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	double2 double2::operator*= (double2 r) {
		x *= r.x;
		y *= r.y;
		return *this;
	}
	
	// componentwise arithmetic operator
	double2 double2::operator/= (double2 r) {
		x /= r.x;
		y /= r.y;
		return *this;
	}
	
	//// arthmethic ops
	
	
	double2 operator+ (double2 v) {
		return double2(+v.x, +v.y);
	}
	
	double2 operator- (double2 v) {
		return double2(-v.x, -v.y);
	}
	
	double2 operator+ (double2 l, double2 r) {
		return double2(l.x + r.x, l.y + r.y);
	}
	
	double2 operator- (double2 l, double2 r) {
		return double2(l.x - r.x, l.y - r.y);
	}
	
	double2 operator* (double2 l, double2 r) {
		return double2(l.x * r.x, l.y * r.y);
	}
	
	double2 operator/ (double2 l, double2 r) {
		return double2(l.x / r.x, l.y / r.y);
	}
	
	//// bitwise ops
	
	
	//// comparison ops
	
	
	// componentwise comparison returns a bool vector
	bool2 operator< (double2 l, double2 r) {
		return bool2(l.x < r.x, l.y < r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator<= (double2 l, double2 r) {
		return bool2(l.x <= r.x, l.y <= r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator> (double2 l, double2 r) {
		return bool2(l.x > r.x, l.y > r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator>= (double2 l, double2 r) {
		return bool2(l.x >= r.x, l.y >= r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator== (double2 l, double2 r) {
		return bool2(l.x == r.x, l.y == r.y);
	}
	
	// componentwise comparison returns a bool vector
	bool2 operator!= (double2 l, double2 r) {
		return bool2(l.x != r.x, l.y != r.y);
	}
	
	// vectors are equal, equivalent to all(l == r)
	bool equal (double2 l, double2 r) {
		return all(l == r);
	}
	
	// componentwise ternary (c ? l : r)
	double2 select (bool2 c, double2 l, double2 r) {
		return double2(c.x ? l.x : r.x, c.y ? l.y : r.y);
	}
	
	//// misc ops
	
	// componentwise absolute
	double2 abs (double2 v) {
		return double2(abs(v.x), abs(v.y));
	}
	
	// componentwise minimum
	double2 min (double2 l, double2 r) {
		return double2(min(l.x,r.x), min(l.y,r.y));
	}
	
	// componentwise maximum
	double2 max (double2 l, double2 r) {
		return double2(max(l.x,r.x), max(l.y,r.y));
	}
	
	// componentwise clamp into range [a,b]
	double2 clamp (double2 x, double2 a, double2 b) {
		return min(max(x,a), b);
	}
	
	// componentwise clamp into range [0,1] also known as saturate in hlsl
	double2 clamp (double2 x) {
		return min(max(x, double(0)), double(1));
	}
	
	// get minimum component of vector, optionally get component index via min_index
	double min_component (double2 v, int* min_index) {
		int index = 0;
		double min_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] <= min_val) {
				index = i;
				min_val = v.arr[i];
			}
		}
		if (min_index) *min_index = index;
		return min_val;
	}
	
	// get maximum component of vector, optionally get component index via max_index
	double max_component (double2 v, int* max_index) {
		int index = 0;
		double max_val = v.x;	
		for (int i=1; i<2; ++i) {
			if (v.arr[i] >= max_val) {
				index = i;
				max_val = v.arr[i];
			}
		}
		if (max_index) *max_index = index;
		return max_val;
	}
	
	
	// componentwise floor
	double2 floor (double2 v) {
		return double2(floor(v.x), floor(v.y));
	}
	
	// componentwise ceil
	double2 ceil (double2 v) {
		return double2(ceil(v.x), ceil(v.y));
	}
	
	// componentwise round
	double2 round (double2 v) {
		return double2(round(v.x), round(v.y));
	}
	
	// componentwise floor to int
	int64v2 floori (double2 v) {
		return int64v2(floori(v.x), floori(v.y));
	}
	
	// componentwise ceil to int
	int64v2 ceili (double2 v) {
		return int64v2(ceili(v.x), ceili(v.y));
	}
	
	// componentwise round to int
	int64v2 roundi (double2 v) {
		return int64v2(roundi(v.x), roundi(v.y));
	}
	
	// componentwise pow
	double2 pow (double2 v, double2 e) {
		return double2(pow(v.x,e.x), pow(v.y,e.y));
	}
	
	// componentwise wrap
	double2 wrap (double2 v, double2 range) {
		return double2(wrap(v.x,range.x), wrap(v.y,range.y));
	}
	
	// componentwise wrap
	double2 wrap (double2 v, double2 a, double2 b) {
		return double2(wrap(v.x,a.x,b.x), wrap(v.y,a.y,b.y));
	}
	
	
	//// Angle conversion
	
	
	// converts degrees to radiants
	double2 radians (double2 deg) {
		return deg * DEG_TO_RADd;
	}
	
	// converts radiants to degrees
	double2 degrees (double2 deg) {
		return deg * DEG_TO_RADd;
	}
	
	// linear interpolation
	// like getting the output of a linear function
	// ex. t=0 -> a ; t=1 -> b ; t=0.5 -> (a+b)/2
	double2 lerp (double2 a, double2 b, double2 t) {
		return t * (b - a) + a;
	}
	
	// linear mapping
	// sometimes called inverse linear interpolation
	// like getting the x for a y on a linear function
	// ex. map(70, 0,100) -> 0.7 ; map(0.5, -1,+1) -> 0.75
	double2 map (double2 x, double2 in_a, double2 in_b) {
		return (x - in_a) / (in_b - in_a);
	}
	
	// linear remapping
	// equivalent of lerp(out_a, out_b, map(x, in_a, in_b))
	double2 map (double2 x, double2 in_a, double2 in_b, double2 out_a, double2 out_b) {
		return lerp(out_a, out_b, map(x, in_a, in_b));
	}
	
	//// Various interpolation
	
	
	// standard smoothstep interpolation
	double2 smoothstep (double2 x) {
		double2 t = clamp(x);
		return t * t * (3.0 - 2.0 * t);
	}
	
	// 3 point bezier interpolation
	double2 bezier (double2 a, double2 b, double2 c, double t) {
		double2 d = lerp(a, b, t);
		double2 e = lerp(b, c, t);
		double2 f = lerp(d, e, t);
		return f;
	}
	
	// 4 point bezier interpolation
	double2 bezier (double2 a, double2 b, double2 c, double2 d, double t) {
		return bezier(
					  lerp(a, b, t),
					  lerp(b, c, t),
					  lerp(c, d, t),
					  t
			   );
	}
	
	// 5 point bezier interpolation
	double2 bezier (double2 a, double2 b, double2 c, double2 d, double2 e, double t) {
		return bezier(
					  lerp(a, b, t),
					  lerp(b, c, t),
					  lerp(c, d, t),
					  lerp(d, e, t),
					  t
			   );
	}
	
	//// Vector math
	
	
	// magnitude of vector
	double length (double2 v) {
		return sqrt((double)(v.x * v.x + v.y * v.y));
	}
	
	// squared magnitude of vector, cheaper than length() because it avoids the sqrt(), some algorithms only need the squared magnitude
	double length_sqr (double2 v) {
		return v.x * v.x + v.y * v.y;
	}
	
	// distance between points, equivalent to length(a - b)
	double distance (double2 a, double2 b) {
		return length(a - b);
	}
	
	// normalize vector so that it has length() = 1, undefined for zero vector
	double2 normalize (double2 v) {
		return double2(v) / length(v);
	}
	
	// normalize vector so that it has length() = 1, returns zero vector if vector was zero vector
	double2 normalizesafe (double2 v) {
		double len = length(v);
		if (len == double(0)) {
			return double(0);
		}
		return double2(v) / double2(len);
	}
	
	// dot product
	double dot (double2 l, double2 r) {
		return l.x * r.x + l.y * r.y;
	}
	
	// 2d cross product hack for convenient 2d stuff
	// same as cross({T.name[:-2]}3(l, 0), {T.name[:-2]}3(r, 0)).z,
	// ie. the cross product of the 2d vectors on the z=0 plane in 3d space and then return the z coord of that (signed mag of cross product)
	double cross (double2 l, double2 r) {
		return l.x * r.y - l.y * r.x;
	}
	
	// rotate 2d vector counterclockwise 90 deg, ie. double2(-y, x) which is fast
	double2 rotate90 (double2 v) {
		return double2(-v.y, v.x);
	}
}

